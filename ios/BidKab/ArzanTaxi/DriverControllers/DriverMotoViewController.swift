//
//  DriverMotoViewController.swift
//  ArzanTaxi
//
...
class DriverMotoViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, DriverMotoControllerDelegate {
...
//        navigationItem.rightBarButtonItem = UIBarButtonItem(image: langImage, style: .plain, target: self, action: #selector(changeLanguage))
        ...
        collectionView.register(DriverMotoOrdersCell.self, forCellWithReuseIdentifier: ordersCellID)
        collectionView.register(DriverMotoCreateCell.self, forCellWithReuseIdentifier: createCellID)
        collectionView.register(DriverMotoMyAutoCell.self, forCellWithReuseIdentifier: myOrdersCellID)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.item == 0 {
            let ordersCell = collectionView.dequeueReusableCell(withReuseIdentifier: ordersCellID, for: indexPath) as! DriverMotoOrdersCell
            
            cell = ordersCell
        } else if indexPath.item == 1 {
            let autoCell = collectionView.dequeueReusableCell(withReuseIdentifier: createCellID, for: indexPath) as! DriverMotoCreateCell
            autoCell.delegate = self
            
            cell = autoCell
        } else if indexPath.item == 2 {
            let myAutoCell = collectionView.dequeueReusableCell(withReuseIdentifier: myOrdersCellID, for: indexPath) as! DriverMotoMyAutoCell
            cell = myAutoCell
        }
        return cell
    }
