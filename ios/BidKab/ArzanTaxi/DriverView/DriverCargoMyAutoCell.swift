...
class DriverCargoMyAutoCell: RefresherCell, UITableViewDelegate, UITableViewDataSource, OptionDelegate {
    ...
    func loadItems() {
        ...
        let url = Constant.api.driver_cargo_my_order + token!
    ...
    func optionButton(id: Int) {
        ...
        let activate = UIAlertAction(title: "Активировать", style: .default) { (action) in
            let body : Parameters = [
                "token" : token,
                "id" : "\(id)"
            ]
        ...
        let deactivate = UIAlertAction(title: "Деактивировать", style: .default) { (action) in
            let body : Parameters = [
                "token" : token,
                "id" : "\(id)"
            ]
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if items[indexPath.row].status! == 0 {
            cell.status = Helper.createOrderLabel(image: #imageLiteral(resourceName: "checkBox"), text: "Not Active", color: .red)
        } else {
            cell.status = Helper.createOrderLabel(image: #imageLiteral(resourceName: "checkBox"), text: "Active", color: .green)
        }
